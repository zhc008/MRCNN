...
class net(nn.Module):
    ...
    def get_results_modified(self, img_shape, detections, detection_masks, box_results_list=None, return_masks=True):
        ...
        batch_synapse_locs = [[] for _ in range(img_shape[0])]
        # print("multiple_masks shape: ")
        # print(multiple_masks.shape)
        # loop over batch and unmold detections.
        for ix in range(img_shape[0]):  # batch ix
            # synapse_locs = []
            ...
            if not 0 in detections[ix].shape:
                ...
                if return_masks:
                    ...
                    for i in range(masks.shape[0]): #masks per this batch instance/element/image
                        ...
                        current_mask = np.zeros(full_mask.shape, dtype=np.uint32)
                        current_mask[full_mask > 0.5] = 1
                        # print(f"current_mask.shape: {current_mask.shape}")
                        current_mask = self.edit_labels(current_mask)

                        #### find the coordinates of each mask
                        if np.where(current_mask==1)[0].shape[0] == 0:
                            continue
                        batch_synapse_locs[ix].append(np.where(current_mask == 1))

                        #%%
                        # if np.any(current_mask > 0):
                        #     current_prop_area = np.sum(current_mask[current_mask==1])
                        ...
                        #             overlapping_mask = np.zeros(multiple_masks.shape, dtype=np.uint32)
                        #             overlapping_mask[multiple_masks == label] = 1
                        #             # print(f"label:{label}")
                        #             # print(f"overlapping_mask: {np.max(overlapping_mask)}")
                        #             overlapping_prop_area = np.sum(overlapping_mask[overlapping_mask==1])
                        ...
                        #                     smaller = True
                        #                     # print(f"label{label} > label{current_mask}")
                        #                     # break
                        #                 else:
            ...
            seg_probs.append(final_masks)
            # print(synapse_locs)
            # batch_synapse_locs.append(synapse_locs)
        ...
        results_dict['sparse'] = batch_synapse_locs

        return results_dict
    
    def edit_labels2(self, waterim):
        if np.max(waterim)==0:
            return waterim
        ...
        for zz in range(len(waterim)): #this step takes a ~2 mins per 1024x1024x181 image
            ...
            check_area = np.max(zim)
            for i in range(1, check_area+1):
                if np.sum([zim == i]) <= 3:
                    zim[zim==i] = 0
            zim[zim > 0] = 1
            waterim[zz] = zim
        ...
        check_plane = np.max(waterim)
        for i in range(1, check_plane+1):
            num_z_planes = np.unique(np.where(waterim==i)[0])
            if num_z_planes.shape[0] <= 2:
                waterim[waterim==i] = 0
    ...
    def binary_iou(self, A, B):
        ...
        if np.sum(intersection)/np.sum(mask1) > 0.5 or np.sum(intersection)/np.sum(mask2) > 0.5:
            iou_score = 0.9
