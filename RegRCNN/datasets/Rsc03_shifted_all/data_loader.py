    ...
    def __init__(self, cf, logger, subset_ids=None, data_sourcedir=None, mode='train'):
        ...
        for ix, pid in enumerate(pids):
            ...
            if 'class' in self.cf.prediction_tasks:
                self.data[pid]['class_targets'] = np.array(class_targets[ix], dtype='int32')
            else:
                self.data[pid]['class_targets'] = np.ones_like(np.array(class_targets[ix]), dtype='int32')
            if load_exact_gts:
            ...
            if any(['regression' in task for task in self.cf.prediction_tasks]):
                ...
                self.data[pid]["rg_bin_targets"] = np.array([cf.rg_val_to_bin_id(v) for v in rg_targets[ix]], dtype='int32')
                if load_exact_gts:
...
class BatchGenerator(dutils.BatchGenerator):
    ...
    def generate_train_batch(self):
        ...
        for b in range(len(batch_pids)):
            ...
            seg =  np.load(patient['seg'], mmap_mode='r').astype('uint16') ## changed from uint8 to uint16

            (c, y, x, z) = data.shape
            # print(self.cf.pre_crop_size)
            # print(seg.shape)
            if self.cf.dim == 2:
            ...
            if np.any(dim_cropflags):
                ...
                if choose_fg and np.any(seg):
                    # print(self.unique_ts.shape)
                    available_roi_ids = np.unique(seg)[1:]
                    # print(batch_roi_counts.shape)
                    # print(patient[self.balance_target].shape)
                    # print(available_roi_ids)
                    # print(self.unique_ts)
                    for tix in np.argsort(batch_roi_counts):
                        # print(patient)
                        # print(available_roi_ids)
                        elig_roi_ids = available_roi_ids[patient[self.balance_target][available_roi_ids-1] == self.unique_ts[tix]]
                        # elig_roi_ids = available_roi_ids[patient[self.balance_target][:] == self.unique_ts[tix]] ## warning! changed here
                        # print(elig_roi_ids)
                    ...
                    def get_cropped_centercoords(dim):
                        low = np.max((self.cf.pre_crop_size[dim] // 2,
                                      roi_anchor_pixel[dim] - (
                                                  self.cf.patch_size[dim] // 2 - self.crop_margin[dim])))
                ...
                sample_seg_center = {}
                # np.save('/cis/home/zchen163/my_documents/debugging/example_seg', seg) ### changes here
        ...
        batch = {'data': np.array(batch_data), 'seg': np.array(batch_segs).astype('uint16'), ## changed from uint8 to uint16
                 'pid': batch_pids,
        ...
        # np.save('/cis/home/zchen163/my_documents/debugging/batch_segs', batch_segs) ### changes here
        return batch
...
class PatientBatchIterator(dutils.PatientBatchIterator):
    ...
    def generate_train_batch(self, pid=None):
        ...
        data = np.load(patient['data'], mmap_mode='r').astype('float16')[np.newaxis]
        seg =  np.load(patient[self.gt_prefix+'seg']).astype('uint16')[np.newaxis] ### changed from uint8 to uint16
        # print('check')
        ...
        if self.cf.dim == 2:
            ...
            out_seg = np.transpose(seg, axes=(3, 0, 1, 2)).astype('int32')  # (c,y,x,z) to (b=z,c,x,y)
        ...
        if np.any([spatial_shp[ix] > self.patch_size[ix] for ix in range(len(spatial_shp))]):
            ...
            if self.cf.dim == 2:
                ...
                data = data[..., 0]
                seg = seg[..., 0]
            patch_batch = {'data': data.astype('float32'), 'seg': seg.astype('int32'),
...
def create_data_gen_pipeline(cf, patient_data, do_aug=True, **kwargs):
    ...
    if do_aug:
        ...
        spatial_transform = SpatialTransform(patch_size=cf.patch_size[:cf.dim],
                                             ...
                                             random_crop=cf.da_kwargs['random_crop'], p_rot_per_sample=cf.da_kwargs['p_rot_per_sample'])
    ...
    # data_gen.n_filled_threads = 1 ##### changes here, added for debugging
...
if __name__=="__main__":
    ...
    from datasets.Rsc01_t1_2.configs import Configs
    ...
    cf.exp_dir = "/cis/home/zchen163/my_documents/regrcnn_results"
    ...
    for i in range(0):
        ...
        plg.view_batch(cf, ex_batch, out_file="/cis/home/zchen163/my_documents/regrcnn_results/dev_exbatch_{}.png".format(i), show_gt_labels=True, vmin=0, show_info=False)
    ...
    for i in range(1):
        ...
        plg.view_batch(cf, ex_batch, out_file="/cis/home/zchen163/my_documents/regrcnn_results/dev_exvalbatch_{}.png".format(i), show_gt_labels=True, vmin=0, show_info=True)
    ...
    stime = time.time()
    plg.view_batch(cf, ex_batch, show_gt_labels=True, out_file="/cis/home/zchen163/my_documents/regrcnn_results/dev_expatchbatch.png", vmin=0)
