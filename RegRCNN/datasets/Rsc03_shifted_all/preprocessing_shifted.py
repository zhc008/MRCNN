    ...
    for y in range(0, height - size + 1, stride):
        for x in range(0, width - size + 1, stride):
            for z in range(0, zidth - depth + 1, z_stride):
                ...
                raw_shifted = np.copy(raw_t_cut)[y:y+size, x+1:x+1+size, z:z+depth]
                label_shifted = np.copy(label_t_cut)[y:y+size, x+1:x+1+size, z:z+depth]
                ...
                # elif np.unique(label_temp).shape[0] <= 37:      # 64 64 16
                    ...
                    raw_shifted[raw_shifted > 0] = 0
                    raw_shifted[raw_shifted < 0] = 0
                    label_shifted[label_shifted > 0] = 0
                # elif np.unique(label_temp).shape[0] > 380:
                #     continue
                skio.imsave(raw_save_dir + "/crop" + str(sub_index*2-1) + ".tif", arr = raw_temp) # save raw images
                skio.imsave(label_save_dir + "/crop" + str(sub_index*2-1) + ".tif", arr = label_temp) # save label images
                skio.imsave(raw_save_dir + "/crop" + str(sub_index*2) + ".tif", arr = raw_shifted) # save raw shifted images
                skio.imsave(label_save_dir + "/crop" + str(sub_index*2) + ".tif", arr = label_shifted) # save label shifted images
