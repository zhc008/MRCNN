    ...
    for y in range(0, height - size + 1, stride):
        for x in range(0, width - size + 1, stride):
            for z in range(0, zidth - depth + 1, z_stride):
                ...
                raw_shifted1 = np.copy(raw_t_cut)[y:y+size, x+shift_size:x+shift_size+size, z:z+depth]
                label_shifted1 = np.copy(label_t_cut)[y:y+size, x+shift_size:x+shift_size+size, z:z+depth]
                raw_shifted2 = np.copy(raw_t_cut)[y:y+size, x-shift_size:x-shift_size+size, z:z+depth]
                label_shifted2 = np.copy(label_t_cut)[y:y+size, x-shift_size:x-shift_size+size, z:z+depth]
                raw_shifted3 = np.copy(raw_t_cut)[y+shift_size:y+shift_size+size, x:x+size, z:z+depth]
                label_shifted3 = np.copy(label_t_cut)[y+shift_size:y+shift_size+size, x:x+size, z:z+depth]
                raw_shifted4 = np.copy(raw_t_cut)[y-shift_size:y-shift_size+size, x:x+size, z:z+depth]
                label_shifted4 = np.copy(label_t_cut)[y-shift_size:y-shift_size+size, x:x+size, z:z+depth]
