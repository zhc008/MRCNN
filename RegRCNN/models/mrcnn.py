...
import utils.iou_loss as iutils
...
def mask_to_bbox_3d(masks):
    ...
    """
    bboxes = []
    device = masks.device  # Get the device from the input tensor
    for i in range(masks.shape[0]):
        mask = masks[i]
        pos = torch.where(mask > 0)
        if len(pos[0]) == 0:
            # No positive pixels in mask, return an impossible bbox
            bboxes.append([0, 0, 0, 0, 0, 0])
        else:
            z_min, z_max = torch.min(pos[0]), torch.max(pos[0])
            y_min, y_max = torch.min(pos[1]), torch.max(pos[1])
            x_min, x_max = torch.min(pos[2]), torch.max(pos[2])
            bboxes.append([x_min.item(), y_min.item(), z_min.item(), x_max.item(), y_max.item(), z_max.item()])
    
    # Convert list to tensor and ensure it is on the correct device
    bboxes_tensor = torch.tensor(bboxes, dtype=torch.float32, device=device)
    return bboxes_tensor


def compute_mrcnn_ciou_loss(pred_masks, target_masks, target_class_ids):
    """
    ...
    if not 0 in torch.nonzero(target_class_ids > 0).size():
        ...
        pred_bbox = mask_to_bbox_3d(y_pred)
        target_bbox = mask_to_bbox_3d(y_true)
        # print(f"pred_bbox:{pred_bbox}")
        # print(f"target_bbox:{target_bbox}")
        loss = iutils.complete_box_iou_loss_3d(pred_bbox, target_bbox, "mean")
    else:
        loss = torch.FloatTensor([0]).cuda()
...
class net(nn.Module):
    ...
    def train_forward(self, batch, is_validation=False):
        ...
        if self.cf.frcnn_mode:
            ...
            mrcnn_ciou_loss = compute_mrcnn_ciou_loss(mrcnn_pred_mask, target_mask, target_class_ids)
        
        # print(f"mrcnn_bbox_loss: {mrcnn_bbox_loss}")
        # print(f"mrcnn_mask_loss: {mrcnn_mask_loss}")
        # print(f"mrcnn_ciou_loss: {mrcnn_ciou_loss}")

        loss = batch_rpn_class_loss + batch_rpn_bbox_loss +\
               mrcnn_bbox_loss + mrcnn_mask_loss +  mrcnn_class_loss + mrcnn_regressions_loss + mrcnn_ciou_loss
    ...
    # modified function
    def get_results_modified(self, img_shape, detections, detection_masks, box_results_list=None, return_masks=True):
        """
        Restores batch dimension of merged detections, unmolds detections, creates and fills results dict.
        :param img_shape:
        :param detections: shape (n_final_detections, len(info)), where
        ...
        # seg_logits == seg_probs in mrcnn since mask head finishes with sigmoid (--> image space = [0,1])
        seg_probs = []
        # show the masks with separate labels in one image
        multiple_masks = np.zeros((img_shape[2:]))
        # print("multiple_masks shape: ")
        # print(multiple_masks.shape)
        # loop over batch and unmold detections.
        for ix in range(img_shape[0]):
            ...
            if not 0 in detections[ix].shape:
                ...
                # Resize masks to original image size and set boundary threshold.
                if return_masks:
                    # print(f"masks.shape[0]: {masks.shape}")
                    # print(f"scores.shape[0]: {scores.shape}")
                    for i in range(masks.shape[0]): #masks per this batch instance/element/image
                        # skip mask when box score is lower than thresh
                        if scores[i] < 0.99:
                            continue
                        # Convert neural network mask to full size mask
                        if self.cf.dim == 2:
                        ...
                        # print(full_mask.shape)
                        # print("final masks shape: ")
                        # print(final_masks[1].shape)
                        # print(np.where(full_mask > 0.5))
                        # current_mask = np.zeros(full_mask.shape).astype(int)
                        current_mask = np.zeros(full_mask.shape)
                        current_mask[full_mask > 0.5] = 1
                        
                        if np.any(current_mask > 0):
                            current_prop_area = np.sum(current_mask)
                            # if current_prop.area <= 20: # check if the volume of current mask is too small to be a real synapse
                            #     print(current_prop.area)
                            #     continue
                            current_mask = self.edit_labels(current_mask)
                            if np.nonzero(np.unique(current_mask))[0].shape[0] != 1:
                                continue
                            elif np.any(multiple_masks[full_mask > 0.5] > 0):  # check if there's any overlap between masks
                                mask_list = np.unique(multiple_masks[full_mask > 0.5])
                                mask_list = mask_list[np.nonzero(mask_list)]
                                smaller = False
                                for label in mask_list:
                                    # overlapping_mask = np.zeros(multiple_masks.shape).astype(int)
                                    overlapping_mask = np.zeros(multiple_masks.shape)
                                    overlapping_mask[multiple_masks == label] = 1
                                    overlapping_prop_area = np.sum(overlapping_mask)
                                    iou = self.binary_iou(overlapping_mask, current_mask)
                                    if iou > 0.5: # set the threshold for iou
                                        if overlapping_prop_area > current_prop_area:
                                            smaller = True
                                            # break
                                        else:
                                            print(f"new label: {label}")
                                            multiple_masks[multiple_masks == label] = 0
                                    
                                if smaller == False: # if the new mask is not smaller, put it on
                                    # multiple_masks[full_mask > 0.5] = i+1
                                    multiple_masks[current_mask > 0] = i+1
                            else:
                                # multiple_masks[full_mask > 0.5] = i+1    # add indentation if uncomment the above lines 
                                multiple_masks[current_mask > 0] = i+1
                        # multiple_masks[full_mask > 0.7] = i+1  ## try a higher determine threshold here
                        # multiple_masks[full_mask > 0.9] = i+1  ## try a higher determine threshold here
                        # take the maximum seg_logits per class of instances in that class, i.e., a pixel in a class
                        # has the max seg_logit value over all instances of that class in one sample
                        final_masks[class_ids[i]] = np.max((final_masks[class_ids[i]], full_mask), axis=0)
                    final_masks[0] = np.full(final_masks[0].shape, 0.49999999) #effectively min_det_thres at 0.5 per pixel
            ...
            # if no detections were made--> keep full bg mask (zeros).
            seg_probs.append(final_masks)
        # print("len of seg_probs: ")
        # print(len(seg_probs))
        # create and fill results dictionary.
        results_dict = {}
        results_dict['boxes'] = box_results_list
        results_dict['seg_preds'] = np.array(seg_probs)
        results_dict['masks'] = multiple_masks

        return results_dict
    
    def edit_labels(self, waterim):
    ...
    def binary_iou(self, A, B):
        # Convert to binary arrays
        mask1 = np.asarray(A, dtype=bool)
        mask2 = np.asarray(B, dtype=bool)
        ...
        # check for situations that are overlapping a lot but with one inside another
        if np.sum(intersection)/np.sum(mask1) > 0.6 or np.sum(intersection)/np.sum(mask2) > 0.6:
            iou_score = 0.9

        return iou_score
    
    # modified function
    def test_forward_modified(self, batch, return_masks=True):
        """
        test method. wrapper around forward pass of network without usage of any ground truth information.
        prepares input data for processing and stores outputs in a dictionary.
        :param batch: dictionary containing 'data'
        ...
        img = torch.from_numpy(img).float().cuda()
        _, _, _, detections, detection_masks = self.forward(img)
        results_dict = self.get_results_modified(img.shape, detections, detection_masks, return_masks=return_masks)
