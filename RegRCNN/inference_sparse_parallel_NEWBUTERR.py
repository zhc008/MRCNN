...
def parallel_sparse(chunk, width=1024,height=1024,depth_im=303, chunk_size=76):
    ...
    for i in patches:
        try:
            file = open(f"/cis/home/zchen163/my_documents/XTC_data/RSC03_roi1_buffer_sparse/p{i}.pkl", 'rb')
...
if __name__=="__main__":
    ...
    class Args():
        def __init__(self):
            self.dataset_name = "datasets/Rsc01_t1_shifted"
    ...
    sparse_buffer_dir = "/cis/home/gcoste1/MaskReg/MaskRegInference/InfPatchesStorage/" #GABY CHANGED BECAUSE NO PERMISSION TO ACCESS
    ...
    list_folder = ["/cis/home/gcoste1/MaskReg/RSC08/t7"] #GABY changed - folders to Run here

    """ Loop through all the folders and do the analysis!!!"""
    for input_path in list_folder:
        foldername = input_path.split('/')[-2]
        sav_dir = input_path + '/' + foldername + '_output_check4'
        ...
        for i in range(len(examples)):
            ...
            with torch.set_grad_enabled(False):  # saves GPU RAM
                ...
                outputs = pool.map(partial(parallel_sparse, width=width,height=height,depth_im=depth_im,
                                           chunk_size=np.ceil(count/num_processes)), patch_list)
                pool.close()
                pool.join()
