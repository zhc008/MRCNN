...
class BatchGenerator(SlimDataLoaderBase):
    ...
    def __init__(self, cf, data, sample_pids_w_replace=True, max_batches=None, raise_stop_iteration=False, n_threads=None, seed=0):
        ...
        # print(self.eligible_pids)
        # print(self.n_filled_threads)
    ...
    def balance_target_distribution(self, plot=False):
        ...
        for pid in self.sample_stats.index:
            for targ in self.unique_ts:
                ...
                # self.sample_stats.loc[pid, str(targ)] = 0.8  ## warning for changes here
                # self.sample_stats.loc[pid, str(targ)+"_bg"] = 0.2  ## warning for changes here

        self.targ_stats = self.sample_stats.agg(
            ("sum", lambda col: col.sum() / len(self._data)), axis=0, sort=False).rename({"<lambda>": "relative"})
            # ("sum", lambda col: 8/10), axis=0, sort=False).rename({"<lambda>": "relative"})
        # print(self.targ_stats.loc["relative"])
...
def convert_seg_to_bounding_box_coordinates(data_dict, dim, roi_item_keys, get_rois_from_seg=False,
    ...
    # print(data_dict['class_targets'].shape)
    for b in range(data_dict['seg'].shape[0]):
        ...
        if np.sum(data_dict['seg'][b] != 0) > 0:
            # print(np.sum(data_dict['seg'][b] != 0))
            if get_rois_from_seg:
                ...
                # print(n_cands)
            # print(data_dict['seg'][b].shape)
            # for name in roi_item_keys:
            #     if data_dict[name][b].shape[0] == 0:
            #         print('0')
            rois = np.array(
                [(data_dict['seg'][b] == ii) * 1 for ii in range(1, n_cands + 1)], dtype='int16')  # separate clusters ### changed from uint8 to int16
            # print(np.unique(rois))
            # print(rois.shape)
            # np.save('/cis/home/zchen163/my_documents/debugging/example_rois', rois)
            for rix, r in enumerate(rois):
                # if (rix >= data_dict['class_targets'].shape[1]):
                #         continue
                # print(rix)
                if np.sum(r != 0) > 0:  # check if the roi survived slicing (3D->2D) and data augmentation (cropping etc.)
                    ...
                    for name in roi_item_keys:
                        # print(name)
                        # print(data_dict[name][b].shape)
                        # print(rix)
                        p_roi_items_lists[name].append(data_dict[name][b][rix])
                    # print(data_dict[name][0][0])
